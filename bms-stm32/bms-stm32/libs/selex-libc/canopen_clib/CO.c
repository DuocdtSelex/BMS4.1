...
void CO_process(CO* p_co, uint16_t time_diff_ms)
{
	...
	CO_process_tpdo(p_co, time_diff_ms, sync_was);
}
void PDO_process(CO* p_co, uint16_t time_diff_ms){
	bool sync_was;
	sync_was = CO_SYNC_process(&p_co->sync, 1, time_diff_ms);
	CO_process_tpdo(p_co, time_diff_ms, sync_was);

}
void CO_process_tpdo(CO *p_co, uint16_t time_diff_ms, bool sync_was)
{
    for (uint8_t i = 0; i < TPDO_NUMBER; i = i + 2)
    {
    	if(p_co->tpdos[i].p_sync->sync_counter != p_co->tpdos[i].comm_parameter.transmission_type)
    	{
    		continue;
    	}
    	CO_TPDO_process(&p_co->tpdos[i],time_diff_ms, sync_was);
    	//delay_ms(1);
        CO_TPDO_process(&p_co->tpdos[i + 1],time_diff_ms, sync_was);
    }
}
...
void CO_set_node_id(CO* p_co, uint8_t node_id)
{
	...
	while(i< TPDO_NUMBER)
	{
		transmission_type = (uint16_t) (5 + j +(node_id - 4)*4);
		if( node_id < 4) transmission_type = 0;
		CO_TPDO_set_transmission_type(&p_co->tpdos[i],transmission_type);
		CO_TPDO_set_transmission_type(&p_co->tpdos[i+1],transmission_type);
		i = i + 2;
		j++;
	}
}
